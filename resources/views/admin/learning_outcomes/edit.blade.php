<x-app-layout>
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md">
        ...
        <form action="{{ route('admin.learning-outcomes.update', $learningOutcome) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <!-- Pilihan Fase -->
                <div>
                    ...
                    <select name="phase_id" id="phase_id" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach ($phases as $phase)
                            <option value="{{ $phase->id }}" {{ old('phase_id', $learningOutcome->phase_id) == $phase->id ? 'selected' : '' }}>
                                {{ $phase->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="subject_id" id="subject_id" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}" {{ old('subject_id', $learningOutcome->subject_id) == $subject->id ? 'selected' : '' }}>
                                {{ $subject->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="element_id" id="element_id" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        ...
                        @foreach ($elements as $element)
                            <option value="{{ $element->id }}" {{ old('element_id', $learningOutcome->element_id) == $element->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="5" required class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $learningOutcome->description) }}</textarea>
                </div>
            </div>
