<x-app-layout>
    <x-slot name="header">
        Monitoring Alur Tujuan Pembelajaran (ATP)
    </x-slot>

    <!-- Panel Filter -->
    <div class="p-6 mb-6 bg-white rounded-lg shadow-md">
        <h3 class="text-lg font-semibold text-gray-800 mb-4">Filter Data</h3>
        <form action="{{ route('headmaster.monitoring.atp') }}" method="GET">
    ...
    <div class="p-6 overflow-hidden bg-white rounded-md shadow-md">
        <div class="overflow-x-auto">
            <table class="min-w-full">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left">Nama ATP</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($teachingFlows as $flow)
                        <tr>
                            ...
                            <td class="px-6 py-4">{{ $flow->user->name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $flow->subject->name }} / {{ $flow->gradeLevel->name }}</td>
                            <td class="px-6 py-4 text-right">
                                <!-- Kita gunakan route 'edit' milik guru untuk melihat detail alurnya -->
                                <a href="{{ route('teacher.teaching-flows.edit', $flow) }}" target="_blank" class="text-indigo-600 hover:text-indigo-900">Lihat Alur</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                                Tidak ada data ATP yang cocok dengan filter.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="mt-4">
            {{ $teachingFlows->appends(request()->query())->links() }}
        </div>
    </div>
</x-app-layout>
